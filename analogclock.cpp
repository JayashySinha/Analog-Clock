 ...
 void main()
{
 ...
 while(1)
 {
 while(ii<14.36)  //for leftmost bob extreme point
  {
  if(kbhit())
     exit(1);
   // second's pixel circle
  for(i=11;i<=17.3;i+=0.105)
   ...
   //time function
       ...
       gettime(&t);   //current time from system
       gotoxy(35,20);
       ...
       int rem=min/12;        // for continous
       k=k+(rem*0.105);       // movement for hour hand
   ...
   exit(1);
   float l,m;
     for(pks=1;pks<=4;pks++)   //pendulum bob
     {
     l=200*cos(ii);
     m=200*sin(ii);
     setcolor(4);
 ...
 line(300,200,x+300,y+200);
    ...
    line(300,200,xh+300,yh+200);
			...
			//for rightmost bob extreme point
    while(ii>13.89)
    {
    if(kbhit())
     exit(1);
